    ...
    internal int GetTVoucherDtlsMaxId(t_voucher_dtls tvd)
         ...
         internal int DeleteInsertVoucherDtls(List<t_voucher_dtls> tvd)
        {
            ...
            string _query="delete t_voucher_dtls"
                        +" Where  brn_cd = {0}  AND voucher_dt = to_date('{1}','dd-mm-yyyy' ) AND "
                        +" voucher_id = {2} ";
            string _query1="Update t_voucher_narration"
                            +" Set narration = {0} ,voucher_type={1}"
                            +" Where  brn_cd = {2}  AND voucher_dt = to_date('{3}','dd-mm-yyyy' ) AND "
                            +" voucher_id = {4} ";
            string _queryIns="INSERT INTO T_VOUCHER_DTLS (BRN_CD, VOUCHER_DT, VOUCHER_ID, ACC_CD, AMOUNT, DEBIT_CREDIT_FLAG, TRANSACTION_TYPE, NARRATION, APPROVAL_STATUS)"
                            +" VALUES ({0},  to_date('{1}','dd-mm-yyyy' ), {2}, {3}, {4}, {5}, {6},  {7}, {8})";
        ...
        internal int DeleteVoucherDtls(List<t_voucher_dtls> tvd)
        {
            ...
            string _query="delete t_voucher_dtls"
                        +" Where  brn_cd = {0}  AND voucher_dt = to_date('{1}','dd-mm-yyyy' ) AND "
                        +" voucher_id = {2} ";
            string _query1="delete T_VOUCHER_NARRATION"
                        +" Where  brn_cd = {0}  AND voucher_dt = to_date('{1}','dd-mm-yyyy' ) AND "
                        +" voucher_id = {2} ";
           using (var connection = OrclDbConnection.NewConnection)
            {              
